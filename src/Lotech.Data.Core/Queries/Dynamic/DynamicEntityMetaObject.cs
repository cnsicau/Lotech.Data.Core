...
{
    public class DynamicEntityMetaObject : DynamicMetaObject
    {
        ...
        public override DynamicMetaObject BindConvert(ConvertBinder binder)
        {
            ...
            else if (binder.ReturnType.IsClass && !binder.ReturnType.IsAbstract && !binder.ReturnType.IsArray && !binder.ReturnType.IsByRef)
            {
                ...
                for (int i = 0; i < fields.Length; i++)
                {
                    ...
                    var value = typeof(Utils.Convert<>).MakeGenericType(
                             fieldMember.MemberType == MemberTypes.Field
                                ? ((FieldInfo)fieldMember).FieldType
                                : ((PropertyInfo)fieldMember).PropertyType
                            )
                           .GetMethod(nameof(Utils.Convert<string>.CreateFromExpression))
                           .Invoke(null, new object[] { fieldValue }) as Expression;
